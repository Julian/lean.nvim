...
describe('Window', function()
  ...
  describe('quit', function()
    it('closes the window', function()
      local window = Window:split()
      local before = Tab:current():windows()
      window:quit()
      assert.is_false(window:is_valid())
      assert.is.equal(#before - 1, #Tab:current():windows())
    end)

    it('closes the tab when the window is the last one in it', function()
      local tab = Tab:new()
      local window = Window:current()
      window:quit()
      assert.is_false(vim.api.nvim_tabpage_is_valid(tab.id))
      assert.is_false(window:is_valid())
    end)
  end)
