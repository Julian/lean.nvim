...
return function(ctx, expr)
  ...
  local function selection_to_child()
    if selection.tag == 'auto' then
      return Html(presentations[1].html, ctx:subsession())
    elseif selection.name ~= 'none' then
      return Html(presentations[selection.name].html, ctx:subsession())
    elseif selection.name == 'none' then
    ...
    children = {
      ...
      Element.select(
        ...
        { ---@type SelectionOpts<SelectedPresentation>
          initial = presentations[selection_name].userName,
        ...
        function(choice)
          ...
          return Element:new { text = choice.userName }
        end
      ),
    },
  }
